...
//        1. Spawn slaves and send i[i] to its corresponding slave.
//         Either send cliques or let slaves calculate them themselves. Send depth, maxDepth, maxWidth. Send a list of slaves for it to use (may be empty).
//        2. Wait until one to report solved or all of them to report not solved.
//        3. a) If one of them reported solved, it should also send all actions made to solve the clique. Apply it. Stop the others. Return true.
//           b) If none reported true, return false. (then depending on depth, maxDepth and maxWidth will be increased to try again.
